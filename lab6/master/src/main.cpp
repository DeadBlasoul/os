...
namespace {
    ...
    public:
        explicit network(zmq::context_t& context)
            : context_ {context} {
    ...
    private:
        auto static constexpr remote_public_commands = std::array {
            "start"sv,
            "stop"sv,
            "time"sv,
        };
        ...
        auto x_create_root_node(std::int64_t const id) -> response {
            ...
            try {
                ...
                port_pool_.insert(port);
                x_throw_internal("cannot create new root node");
            }
        }
        ...
        /// Sends message to every root node until valuable response
        /**
    ...
    private:
        ...
        static auto verify_exec(std::string_view const command) -> void {
            if (!std::any_of(
                remote_public_commands.begin(),
                remote_public_commands.end(),
...
auto main() -> int try {
    ...
    auto static process_reply = [](network::response const& response) {
        if (response.error != network::response::error_type::ok) { }
    ...
    auto runner  =
        ...
        .assign_or_update({
            ...
            .callback = [&](commandline::command_runner::argv_t const& argv) {
                ...
                auto const rep = network.create(id, parent);
                process_reply(rep);
            }
        })
        .assign_or_update({
            ...
            .callback = [&](commandline::command_runner::argv_t const& argv) {
                ...
                auto const rep = network.remove(id);
                process_reply(rep);
            }
        })
        .assign_or_update({
            ...
            .callback = [&](commandline::command_runner::argv_t const& argv) {
                ...
                auto const rep = network.exec(id, command);
                process_reply(rep);
            }
        })
        .assign_or_update({
            ...
            .callback = [&](commandline::command_runner::argv_t const& argv) {
                ...
                auto const rep = network.ping(id);
                process_reply(rep);
            }
        });
