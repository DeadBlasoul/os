...
int create_win32_piped_process(const char* const cmd_line, process* const cp, pipe_pair* const in, pipe_pair* const out);
...
int create_win32_piped_process(const char* const cmd_line, process* const cp, pipe_pair* const in, pipe_pair* const out) {
    ...
    cp->h_process = (system_handle*)piProcInfo.hProcess;
    cp->h_thread = (system_handle*)piProcInfo.hThread;
    return 0;
}
